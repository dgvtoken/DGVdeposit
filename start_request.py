...
from config.common_config import ENCRYPT_KEY, TIME_MULTIPLE, SLEEP_TIME_REQUEST
from config.common_config import gate_db_conn as GATE_DB_CONN
from config.one_config import COIN_CONFIG
from core import deposit_request
...
def start():
    try:
        while True:
            monitor = deposit_request.DepositRequest()
            monitor.runloop()

            time.sleep(SLEEP_TIME_REQUEST)  # 休眠指定时间后继续执行
